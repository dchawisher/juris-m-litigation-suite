...
function getAnnotations(doc) {
	...
	let note = ''
	let lastNoteId = '' //avoid consecutive duplicates
	for (s of noteSections) {//TODO make this more robust against improper colors in the future
		let sNote = ''
		for (h of highlights) {
			if (s[1].indexOf(h.color) > -1) {
				if (h.id != lastNoteId) { //avoid consecutive duplicates
					lastNoteId = h.id
					if (comments[h.id]) {
